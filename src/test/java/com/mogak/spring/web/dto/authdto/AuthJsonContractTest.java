...
class AuthJsonContractTest {
    ...
    @Test
    @DisplayName("AuthWithdrawResponse는 isDeleted JSON 이름을 유지한다")
    void withdrawResponseUsesExpectedFieldName() throws Exception {
        AuthWithdrawResponse response = new AuthWithdrawResponse(true);

        String json = objectMapper.writeValueAsString(response);

        assertThat(json).contains("\"isDeleted\":true");
        assertThat(objectMapper.readValue(json, AuthWithdrawResponse.class).isDeleted()).isTrue();
    }
}
